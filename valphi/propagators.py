...
def val_phi_as_weight_constraints(symbolic_atoms: clingo.SymbolicAtoms, output_node: str,
                                  val_phi: List[int], ordered_encoding: bool = False) -> List[str]:
    input_nodes = set()
    input_weight = {}

    for s in symbolic_atoms.by_signature("sub_type", 3):
        concept1, concept2, weight = s.symbol.arguments
        if concept1.name == output_node:
            input_nodes.add(concept2)
            validate("int weights", float(weight.string).is_integer(), equals=True)
            input_weight[concept2] = int(float(weight.string))

    agg_set = []
    for node in input_nodes:
        for value in range(len(val_phi)):
            if ordered_encoding:
                agg_set.append(f"{input_weight[node]},{node},{value+1} : eval_ge({node},{value+1})")
            else:
                agg_set.append(f"{input_weight[node] * (value+1)},{node},{value+1} : eval({node},{value+1})")
    agg_set = '; '.join(agg_set)

    def constraint(output_node_value: str, lower_bound: Optional[int], upper_bound: Optional[int]):
        result = []
        for direction in ["=>", "<="]:
            result.append(
                ":- " + ("" if direction == "=>" else "not ") +
                (f"{lower_bound} < " if lower_bound is not None else "") +
                "#sum{" + agg_set + "}" +
                (f" <= {upper_bound}" if upper_bound is not None else "") +
                f", " + ("" if direction == "<=" else "not ") + f"{output_node_value}."
            )
        return result

    res = constraint(f"eval({output_node},0)", None, int(val_phi[0]))
    for index in range(len(val_phi)):
        res += constraint(f"eval({output_node},{index + 1})", val_phi[index],
                          val_phi[index + 1] if index + 1 < len(val_phi) else None)
    return res
